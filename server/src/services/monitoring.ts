...
type Discovery = typeof discoverDevices;

async function performMonitoringTick(userId: string, clientIp: string, discover: Discovery) {
  ...
  for (const network of networks) {
    ...
    const devices = await discover(userId, network.id, clientIp);
    for (const device of devices) {
      if (!beforeIds.has(device.id)) {
        const risk = calculateRisk({ newDevice: true });
        await prisma.device.update({ where: { id: device.id }, data: { riskLevel: risk.level } });
...
export async function runMonitoringTick(userId: string, clientIp: string, discover: Discovery = discoverDevices) {
  if (runningTicks.has(userId)) return;
  runningTicks.add(userId);
  try {
    await performMonitoringTick(userId, clientIp, discover);
  } finally {
    runningTicks.delete(userId);
  }
}
...
export async function startMonitoring(userId: string, intervalSeconds: number, clientIp: string, discover: Discovery = discoverDevices) {
  ...
  const timer = setInterval(() => void runMonitoringTick(userId, clientIp, discover).catch(() => undefined), interval * 1000);
  timers.set(userId, timer);
  void runMonitoringTick(userId, clientIp, discover).catch(() => undefined);
...
export async function updateAlert(userId: string, id: string, status: "OPEN" | "ACKNOWLEDGED" | "RESOLVED", clientIp: string) {
  ...
  const valid = (alert.status === "OPEN" && status === "ACKNOWLEDGED") || (alert.status === "ACKNOWLEDGED" && status === "RESOLVED");
  if (!valid) throw Errors.validation(`Invalid alert transition: ${alert.status} -> ${status}.`);
...
export function activeMonitoringCount() {
  return timers.size;
}

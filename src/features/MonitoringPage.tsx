...
import { monitoringService, type SecurityAlert, type SecurityEvent } from "@/services/monitoringService";
...
export function MonitoringPage() {
  ...
  async function load() {
    try { const [nextEvents, nextAlerts] = await Promise.all([monitoringService.events(), monitoringService.alerts()]); setEvents(nextEvents); setAlerts(nextAlerts); } catch (cause) { setError(cause instanceof Error ? cause.message : "Monitoring data is unavailable."); } finally { setLoading(false); }
  }
  useEffect(() => { void load(); }, []);
